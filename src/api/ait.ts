...
const MOCK_MESSAGES: Record<string, Message[]> = {
  ...
  // ── c6: 짧은 QA: 배포 환경 변수 (1 user turn, no branches) ──────────
  c6: [
    {
  ...
  // ── c4: 프론트엔드 에러 처리 전략 (5 user turns, 3 branches) ───────
  c4: [
    {
  ...
  // ── b4: API 실패 UX (forks from c4 T1, 1 branch-specific turn) ─────
  b4: [
    {
  ...
  // ── b5: 폼 검증 케이스 (forks from c4 T4, 3 branch-specific turns) ──
  b5: [
    {
  ...
  // ── b6: 접근성 점검 (forks from c4 T5, intro-only branch) ─────────
  b6: [
    {
  ...
  // ── c5: 데이터베이스 인덱스 설계 (3 user turns, 1 branch) ─────────
  c5: [
    {
  ...
  // ── b7: 쿼리 플랜 해석 (forks from c5 T2, 2 branch-specific turns) ──
  b7: [
    {
  ...
  // ── c7: 긴 답변 렌더링 테스트 (2 user turns, 1 branch) ───────────
  c7: [
    {
  ...
  // ── b8: 표 형식 요약 (forks from c7 T1, 1 branch-specific turn) ───
  b8: [
    {
