...
const PROVIDERS: { provider: AuthLoginProvider; label: string; className: string; icon: React.ReactNode }[] = [
  {
    provider: 'google',
    label: 'Google로 계속하기',
...
const SocialLoginButtons: React.FC<SocialLoginButtonsProps> = ({ onSuccess }) => {
  const { login } = useAuth();
  const [pending, setPending] = useState<AuthLoginProvider | null>(null);

  const handleLogin = async (provider: AuthLoginProvider) => {
    if (pending) return;
    setPending(provider);
